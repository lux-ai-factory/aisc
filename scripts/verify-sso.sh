...
loc=$(curl -s -b "$J" -c "$J" --max-time 20 -o /dev/null -D - \
  "http://localhost:8081/realms/aisc/protocol/openid-connect/auth?client_id=aisc-webapp&redirect_uri=http%3A%2F%2Flocalhost%2F&response_type=code&scope=openid&prompt=none" \
